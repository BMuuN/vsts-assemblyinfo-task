...
describe('Net Core - Task Tests', function() {
    ...
    it('should succeed and set output variables', (done: Mocha.Done) => {
        ...
        assert.strictEqual(tr.errorIssues.length, 0, 'should have no errors');
        assert.strictEqual(tr.stdout.indexOf('##vso[task.setvariable variable=Version;isOutput=true;issecret=false;]2018.11.') > -1, true, 'Version output variable set');
        assert.strictEqual(tr.stdout.indexOf('##vso[task.setvariable variable=FileVersion;isOutput=true;issecret=false;]1990.03.') > -1, true, 'FileVersion output variable set');
        assert.strictEqual(tr.stdout.indexOf('##vso[task.setvariable variable=InformationalVersion;isOutput=true;issecret=false;]2.3.4-prerelease') > -1, true, 'InformationalVersion output variable set');
        assert.strictEqual(tr.stdout.indexOf('##vso[task.setvariable variable=PackageVersion;isOutput=true;issecret=false;]9.8.7-beta5') > -1, true, 'PackageVersion output variable set');
